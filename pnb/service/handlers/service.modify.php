...
function modify_institutions_fields($data) {
	...
	foreach($data as $k => $v) { // iterate over field ids
		$query = 'UPDATE `phonebook_api`.`institutions_fields` SET';
...
function modify_institutions_fields_groups($data) {
	...
	$set = array();
	foreach($data as $k => $v) { // iterate over field ids
		$query = 'UPDATE `phonebook_api`.`institutions_fields_groups` SET';
		foreach($v as $k2 => $v2) { // iterate over column changes for the specific field id
			$k2 = strtolower(trim($k2));
			$name = $db->Escape($k2);
			switch($k2) {
...
function modify_members_fields($data) {
	...
	$set = array();
	foreach($data as $k => $v) { // iterate over field ids
		$query = 'UPDATE `phonebook_api`.`members_fields` SET';
		foreach($v as $k2 => $v2) { // iterate over column changes for the specific field id
			$k2 = strtolower(trim($k2));
			$name = $db->Escape($k2);
			switch($k2) {
...
function modify_members_fields_groups($data) {
	...
	$set = array();
	foreach($data as $k => $v) { // iterate over field ids
		$query = 'UPDATE `phonebook_api`.`members_fields_groups` SET';
		foreach($v as $k2 => $v2) { // iterate over column changes for the specific field id
			$k2 = strtolower(trim($k2));
			$name = $db->Escape($k2);
			switch($k2) {
