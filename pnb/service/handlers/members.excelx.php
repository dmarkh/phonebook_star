...
function members_excelx_handler($params) {
  ...
  $query = 'SELECT * FROM `phonebook_api`.`institutions` WHERE `status` = "active"';
  $inst = $db->Query($query);

  $query = 'SELECT * FROM `phonebook_api`.`members`'.$wherestatus;
  $mem = $db->Query($query);

  $inst_fields = array();
  foreach(array('string','int','date') as $k => $v) {
    $query = 'SELECT * FROM `phonebook_api`.`institutions_data_'.$v.'s`';
    $res = $db->Query($query);
  ...
  header('Content-Disposition: attachment;filename="phonebook-'.time(0).'.xlsx"');
  header('Cache-Control: max-age=0');

  echo $writer->writeToString();

//	$xls = new Excel_XML('UTF-8', false, 'STAR Collaboration members');
//	$xls->addArray($out);
