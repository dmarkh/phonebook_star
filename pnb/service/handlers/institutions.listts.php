...
function institutions_listts_handler($params) {
  ...
  $query = 'SELECT * FROM `phonebook_api`.`institutions` WHERE 1';
  $ins = $db->Query($query);
  ...
  $query = 'SELECT *, UNIX_TIMESTAMP(`value_to_date`) AS uvtd FROM `phonebook_api`.`institutions_history`.WHERE UNIX_TIMESTAMP(`date`) <= '.$ts.' ORDER BY `date` ASC';
  $hist = $db->Query($query);
