...
function institutions_get_handler($params) {
  ...
  $query = 'SELECT * FROM `phonebook_api`.`institutions` WHERE `id` = '.$id.' LIMIT 1';
  $inst = $db->Query($query);
  if (!empty($inst)) {
  	$inst = $inst[0];
  }

  $query = 'SELECT * FROM `phonebook_api`.`institutions_fields`';
  ...
  foreach(array('string','int','date') as $k => $v) {
  	$query = 'SELECT institutions_fields_id as field_id, value as field_value FROM `phonebook_api`.`institutions_data_'.$v.'s` WHERE institutions_id = '.$id;
	$res = $db->Query($query);
