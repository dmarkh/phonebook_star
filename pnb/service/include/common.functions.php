...
function get_institutions_fields() {
    ...
    $query = 'SELECT t1.* FROM `phonebook_api`.`institutions_fields` t1, `phonebook_api`.`institutions_fields_groups` t2 WHERE t1.group = t2.id  ORDER BY t2.weight ASC, t1.`weight` ASC';
    $res = $db->Query($query);
    $fields = array();
    foreach($res as $k => $v) {
        $fields[$v['id']] = $v;
...
function get_members_fields() {
    $db =& ServiceDb::Instance('phonebook_api');
    $query = 'SELECT t1.* FROM `phonebook_api`.`members_fields` t1, `phonebook_api`.`members_fields_groups` t2 WHERE t1.group = t2.id ORDER BY t2.weight ASC, t1.`weight` ASC';
    $res = $db->Query($query);
    $fields = array();
    foreach($res as $k => $v) {
        $fields[$v['id']] = $v;
...
function get_institutions_fieldgroups() {
    $query = 'SELECT * FROM `phonebook_api`.`institutions_fields_groups` ORDER BY `weight` ASC';
    $db =& ServiceDb::Instance('phonebook_api');
    $res = $db->Query($query);
    $groups = array();
    foreach($res as $k => $v) {
...
function get_members_fieldgroups() {
    $query = 'SELECT * FROM `phonebook_api`.`members_fields_groups` ORDER BY `weight` ASC';
    $db =& ServiceDb::Instance('phonebook_api');
    $res = $db->Query($query);
    $groups = array();
    foreach($res as $k => $v) {
