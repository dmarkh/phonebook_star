...
RCLIENT.Phonebook = function () {
	...
	this.tabCounter = 4;
        ...
        this.service_url = 'https://www.star.bnl.gov/public_pnb/service/index.php';
...
RCLIENT.Phonebook.prototype = {
	...
	initialize : function() {
        ...
        service.md5interval = setInterval( function() {
            $.ajax({
                url: 'https://www.star.bnl.gov/public_pnb/client/md5.php?'+Date.now(),
                type: 'GET',
                processData: false,
                success: function(data) {
                    if ( service.md5sum == undefined ) { service.md5sum = data.trim(); }
						...
						console.log('old hash: ' + service.md5sum + ', new hash: ' + data.trim() );
                    }
                }
            });
        }, 15000);
		...
		this.get_institutions_fields_groups(function() {
			console.log('got institution fields');
		...
		this.get_institutions('all', function() {
			console.log('got institutions');
			var val = $('#loading-progress').progressbar("value") + 14.3;
			$('#loading-progress').progressbar({ value: val });
		});

		this.get_members_fields_groups(function(){
			console.log('got member fields groups');
			var val = $('#loading-progress').progressbar("value") + 14.3;
			$('#loading-progress').progressbar({ value: val });
		    service.get_members_fields( function(){
				console.log('got member fields');
				var val = $('#loading-progress').progressbar("value") + 14.3;
				$('#loading-progress').progressbar({ value: val });
			});
		});
    ...
    display_worldmap: function() {
        ...
        $('body').append('<div id="worldmapdialog" title="STAR Collaboration, Institution Locations over the World"><div id="worldmap" style="height: 100%; width: 100%"></div></div>');
	...
	display_institution_details: function(id) {
		...
		var content = '<p style="padding: 10px;"><img src="https://www.star.bnl.gov/include/graphics/sectionTitleStar.gif" border=0> <strong><a href="#iid:'+id+'">'+inst_full_name+'</a></strong><br>';
		...
		if (inst_country != undefined && inst_country != '') {
			content += '<br>' + inst_country + ' <br>';// + '<img src="images/flags_iso_3166/16/'+inst_country_code.toLowerCase()+'.png" style="vertical-align: middle;"> '+ '<br>';
		}
		content += '</p>';
		...
		content += '<p style="padding: 10px;">Institution representative on STAR Council is: ';
		var field = this.members[inst_council_rep_id]['fields'];
		var name_first = field[1], name_last_unmodified = field[3], name_last = field[3];
		content += '<span style="cursor: pointer;" onClick="client.display_member_details('+inst_council_rep_id+')"><strong>' + name_first + ' ' + name_last + '</strong></span>';
		content += '</p>';
		}
	...
	get_institution_list_details: function( inst_id ) {
		var service = this;
		...
		return { 'name': inst_full_name, 'addr1': inst_addr1, 'addr2': inst_addr2, 'city': inst_city,
					'country': inst_country, 'country_code': inst_country_code, 'state': inst_state, 'zip': inst_zip };
	},
	...
	display_member_details: function( id ) {
		...
		var mem_rcflogin = false || fields[ this.find_field_id_members('rcf_login') ];

		content += '<strong><big><a href="#mid:'+id+'">' + mem_name_first + ' ' + mem_name_last + '</a></big></strong><br>';
		...
		var extra_inst_id = fields[this.find_field_id_members('extra_institution_id')];
		var institutions_list = [ inst_id ];
		if ( extra_inst_id && extra_inst_id.length > 0 ) {
			var extra_insts = extra_inst_id.split(',');
			for ( var j = 0; j < extra_insts.length; j++ ) { extra_insts[j] = parseInt( extra_insts[j], 10); }
			institutions_list = institutions_list.concat( extra_insts );
		}

		for ( var i = 0; i < institutions_list.length; i++ ) {
			var details = service.get_institution_list_details( institutions_list[i] );

			var inst_full_name = details.name;
			var inst_addr1 = details.addr1;
			var inst_addr2 = details.addr2;
			var inst_city = details.city;
			var inst_country = details.country;
			var inst_country_code = details.country_code;
			var inst_state = details.state;
			var inst_zip = details.zip;
		...
		if ( mem_rcflogin ) {
		  content += '<br><input type="button" id="edit-details-'+id+'" value="EDIT RECORD"/>';
		}
		...
		$('#edit-details-'+id).off('click').on('click', function() {
		  $('#edit-form-'+id).remove();
		  text = '<fieldset id="edit-form-'+id+'"><legend>RCF Credentials for '+mem_name_first+' '+mem_name_last+'</legend>' +
			  '<form autocomplete="off">' + 
			  '<label for="rcflogin-'+id+'">RCF Login</label><br>' +
			  '<input type="text" id="rcfl-'+id+'" value="" placeholder="RCF Login"><br><br>' +
			  '<label for="rcfpass-'+id+'">RCF Password</label><br>' +
			  ' <input type="password" id="rcfp-'+id+'" value="" placeholder="RCF Pass" autocomplete="off"><br><br>' +
			  ' <input type="button" id="edit-submit-'+id+'" value = "Load Data">' +
			  ' </form></fieldset>';
		  $('#'+tabid).append(text);
			$('#rcfl-'+id).text('');
			$('#rcfp-'+id).text('');

		  $('#edit-submit-'+id).off('click').on('click', function() {
			login = $('#rcfl-'+id).val();
			pass  = $('#rcfp-'+id).val();
			$('#edit-form-'+id).remove();
			memurl = service.service_url+'?q=/members/getkrb/id:'+id+'/rl:'+encodeURIComponent(xor_string(base64_encode(login), 'S'))+'/rp:'+encodeURIComponent(xor_string(base64_encode(pass), 'S'))+'/?ts='+Date.now();
			$.getJSON(memurl, function( data, status, xhr ) {
				//console.log(data);
				if ( data.error ) { alert('ERROR: ' + data.message); return; }
				fields = data.fields;
				...
				$( "#edit-member-dialog" ).dialog({
					...
					buttons: {
						"Update Information": function() {
							...
							console.log(result);
							$.ajax({
								url: service.service_url+'?q=/members/updatekrb/ts='+Date.now(),
								type: 'POST',
								processData: false,
								data: JSON.stringify(result),
								contentType: 'application/json; charset=utf-8',
	...
	display_institutions: function() {
		...
		var header = [ {"bVisible": false}, {"sTitle": this.institutions_fields["1"]["name_desc"]}, 
			...
			{"sTitle":this.institutions_fields["3"]["name_desc"], "sClass": "td_align_center"}, 
		...
		for (var i in this.institutions) {
			...
			data.push([ i, field[1], name_short, name_group, country, region, this.institutions[i]['status']]);
		}
